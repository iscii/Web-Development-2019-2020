...
function iRefs(){
    opReceipts = document.getElementById("orders");
    opLog = document.getElementById("log");
    clStyleReceipts = document.getElementsByClassName("receipts");
    clStyleOrders = document.getElementsByClassName("cOrders");
}
...
function setClassStyle(){ //YOU ARE HERE 1/31/20 -- Add other stats to receipts
    clOrdersWidth = (numOrder * 70) + "%"; //70 and 90 were very rough estimates through trial and error and are by no means good parameters.
    ...
    for(i = 0; i <clStyleOrders.length; i++){
        clStyleOrders[i].style.width = clOrdersWidth;
    }
}

//DISPLAY FUNCTIONS
function dispReceipt(){ //called per order iteration of a simulation
    opReceipts.innerHTML += "<span class = 'receipts'>Order Number " + numOrder + "<br/> +  </span>";
}
function dispTotals(){ //called per simulation
    //opLog.innerHTML += 
}
function display(){
    opReceipts.innerHTML = "<span class = 'simHead'>Sim " + simNum + "</span>"+ opReceipts.innerHTML;
    setClassStyle();
}