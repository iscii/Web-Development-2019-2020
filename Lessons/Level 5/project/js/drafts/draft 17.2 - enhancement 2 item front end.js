...
function iRefs(){
    ...
    opAvgElecSales = document.getElementById("avgelecsales");
    opAvgCashSales = document.getElementById("avgcashsales");
}
...
function dispData(){ //enhancement 2
    ...
    for(i = 0; i < FOODTYPES.length; i++){
        for(x = 0; x < eval(FOODTYPES[i]).length; x++){
            ...
            eval("op" + eval(FOODTYPES[i])[x].split(";")[0] + "Cost").innerHTML = "~$" + (valueHolder[1] / valueHolder[2]).toFixed(2);
            eval("op" + eval(FOODTYPES[i])[x].split(";")[0] + "Count").innerHTML = "~" + parseInt(valueHolder[0] / valueHolder[2]);
