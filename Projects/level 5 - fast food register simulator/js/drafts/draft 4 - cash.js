...
function dCash(){
    ...
    for (i = 3; i >= 0; i--){
        ...
        if(cash[i] > 0){
            ...
            for(x = 3; x >= i; x--){ //skips i = 3
                //console.log("x: " + x);
                //console.log("cash[x]: " + cash[x]);
                for (o = 0; o < (cash[x] - 1); o++){
                    //console.log("o: " + o);
                    cashGiven += VALUES[x];
                    paid.push(VALUES[x]);
                    //console.log(cashGiven);
    ...
    /*
    for (i = 3; i >= 0; i--){ //pays with lowest denomination first
        for (x = 0; x < cash[i]; x++){
            if(cashGiven - orderTotal < 0){
                cashGiven += VALUES[i];
                //console.log("VALUES = " + VALUES[i]);
        ...
        if(cashGiven - orderTotal < 0){ //if the 1s added up cannot pay for the order, add a 5 and add the 1s again.
            cashGiven = 0;
            for (z = (i + 1); z > i; z--){
