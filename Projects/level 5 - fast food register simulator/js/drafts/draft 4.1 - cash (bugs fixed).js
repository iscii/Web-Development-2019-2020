...
function dCash(){
    ...
    var paid = [];
    console.log(cash);
    //objective: take lowest of the values, then keep adding from bottom up. if cannot fulfull, take next value and keep adding from bottom up.
    for (i = 3; i >= 0; i--){
        if(cash[i] > 0){
            cashGiven = VALUES[i];
            paid = [VALUES[i]];
            ...
            for(x = 3; x >= i; x--){
                cashGiven = VALUES[i];              // FIXES a bug where the loop would cycle back to 5
                paid = [VALUES[i]];                 // while continuing the previous cycle.
                if(cashGiven - orderTotal >= 0){    // See ../bugs/1.27.19_-_cash.png
                    console.log(cashGiven);         //
                    console.log("paid: " + paid);   //
                    return;                         //
                }                                   //
                ...
                if(x == i){
                    //console.log("x == i");
                    //console.log("cash x = " + cash[x] + ", x = " + x + ", i = " + i);
                    quantItem = (cash[x] - 1);   
                }                  
                else{
                    //console.log("x != i");
                ...
                //console.log("quantItem = " + quantItem);
                for (o = 0; o < quantItem; o++){
                    cashGiven += VALUES[x];
                    //console.log("+ " + VALUES[x] + ", x = " + x + ", o = " + o + ", quantItem = " + quantItem);
                    paid.push(VALUES[x]);
                    //console.log("paid = " + paid);
                    //console.log(cashGiven);
                    if(cashGiven - orderTotal >= 0){
