...
function play(gridNum)
{
    if(turn && isPlayable(gridNum, MOVES[playerMove]))
    {
        ...
        var pPlay = mGrid[gridNum].split(";");

        pPlay[GRID_1] = MOVES[playerMove];
        mGrid[gridNum] = pPlay.join(";");
        ...
        console.log("p1moved: " + " [" + gridNum + "] " + mGrid);
        if(checkForWin())
        {
...
function cpuPlay()
{
    ...
    if(isPlayable(rand, MOVES[cpuMove]))
    {
        ...
        var cPlay = mGrid[rand].split(";");
        
        cPlay[GRID_1] = MOVES[cpuMove];
        mGrid[rand] = cPlay.join(";");
        ...
        console.log("p2moved:" + " [" + rand + "] " + mGrid);
        checkForWin();
        display();
    }
    else
...
{
    //evaluates mGrid[move].split(";")[0] (which is true/false)
    if (eval(mGrid[move].split(";")[ISLOCKED]))
    {
...
{
    if(mGrid[move].split(";")[GRID_1] == player)
        mGridLock(move);
    else
        return;
}
function mGridLock(move)
{
    //YOU ARE HERE 12/24. Finished two-layer functions. Move onto UI and representing a locked image and stuff.
    var lockGrid = mGrid[move].split(";");

    lockGrid[ISLOCKED] = "true";
    mGrid[move] = lockGrid.join(";");

    console.log("lock " + move + " [ " + mGrid[move] + " ] ");
...
function checkForWin()
{
    for (var move = 0; move < MOVES.length; move++)
    {
        ...
        for (var indexCount = 0; indexCount < 3; indexCount++)
        {
            ...
            {
                if((mGrid[i].split(";")[GRID_1] == MOVES[move]))
                {
                    column++;
